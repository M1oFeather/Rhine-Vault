...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Status-Full%20Implementation-purple?style=flat-square" alt="Status">
</p>
...
当前版本已进入 **Full Implementation Mode**：在正式审批、可解释检索、受限 MCP、Library 与 Phase 6 恢复底座基础上，开始实现原本延后的正式能力，包括可选 OpenAI-compatible embedding provider、局部知识图谱 REST/UI、以及通过 proposal/staging/approval 链路执行的 `.rhine` import apply。

> Full Implementation 不等于放弃架构边界：正式 MemoryNode 仍必须经过可审计审批链路，core-only 默认安装仍保持轻量，向量/图谱/生成文档仍是派生能力而不是知识 authority。
...
| **pypdf / python-docx** | 可选 PDF / DOCX 文档文本抽取 |
...
- **文档导入** — 支持 Markdown / TXT；可选支持 PDF / DOCX 文本抽取，保留文件 hash、heading path 与 line range
...
- **局部知识图谱** — REST 与管理面板可查看 approved 节点之间的关系投影
- **Snapshot Import Apply** — `.rhine` 包可验证后转成 proposal，并可在显式勾选时走审批应用
- **可选 Embedding Provider** — OpenAI-compatible embeddings 仅在显式配置时发起网络请求
...
PDF/DOCX 导入只抽取文本并生成 Capture Proposal，不会直接写入正式 MemoryNode。
...
项目内置一个白面鸮 / Ptilopsis starter seed，可用于第一次体验人物知识库、CMCC 对接上下文和角色资料检索。它不会绕过 Rhine-Vault 的正式工作流：默认创建 proposal 并保存为 staging，人工确认后才进入正式 MemoryNode。

```powershell
.\.venv\Scripts\python.exe -m rhine_vault.seeds.ptilopsis
```

如果你希望本地测试时立即得到可检索的 approved 节点，可以显式批准：

```powershell
.\.venv\Scripts\python.exe -m rhine_vault.seeds.ptilopsis --approve
```
...
data/workspaces/ptilopsis/nodes/*.md
```
...
| `GET /api/documents/importers` | 查看 Markdown/TXT/PDF/DOCX 导入器可用状态 |
| `POST /api/documents/import` | Markdown/TXT/PDF/DOCX 文档导入为候选知识 |
...
- **节点目录与局部图谱** — UI 可浏览已批准节点，并查看节点关系投影
...
<table>
  ...
  <tr>
    ...
    <td>Full Implementation Mode</td>
  </tr>
  <tr>
...
<p align="center">
  <sub>Rhine-Vault 已解除 phase 门禁，后续按可审计、可测试的正式能力切片推进。</sub>
</p>
